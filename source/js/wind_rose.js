(function($){
  ...
  $(function() {
    ...
    var spokeMouseOver = function() {
      var spoke = d3.select(this);
      old_fill = spoke.attr('fill');
      old_stroke = spoke.attr('stroke-width');
      spoke.transition().duration(500).ease('elastic').attr({
        'stroke-width': 2,
        'fill': 'orange'
      });
    };

    var spokeMouseOut = function() {
      var spoke = d3.select(this);
      spoke.transition().duration(500).ease('elastic').attr({
        'stroke-width': old_stroke,
        'fill': old_fill
      });
    };

    d3.selectAll('svg g#stack-data path')
